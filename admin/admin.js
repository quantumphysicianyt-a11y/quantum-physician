...
if(!filtered.length){showToast('All recipients filtered (opted out or at weekly limit)','info');return}
...
/* Pre-compute opted-out + weekly-limited sets for accurate counts */
var _sgOptedOut={};authUsersMap.forEach(function(u){
if(u.raw_user_meta_data&&u.raw_user_meta_data.marketing_opt_in===false)_sgOptedOut[u.email.toLowerCase()]=true});
var _sgWeekCounts={};var _sgLimit=getWeeklyEmailLimit();var _sg7ago=new Date(now-7*864e5).toISOString();
emailTrackingData.forEach(function(t){if(t.email_type==='promotional'&&t.sent_at>=_sg7ago){var e=(t.recipient_email||'').toLowerCase();_sgWeekCounts[e]=(_sgWeekCounts[e]||0)+1}});
var sgFilterEmails=function(emailStr){return emailStr.split('\n').filter(function(e){
var el=e.trim().toLowerCase();return el&&!_sgOptedOut[el]&&(_sgWeekCounts[el]||0)<_sgLimit}).join('\n')};
var sgCountEmails=function(emailStr){return emailStr.split('\n').filter(function(e){
var el=e.trim().toLowerCase();return el&&!_sgOptedOut[el]&&(_sgWeekCounts[el]||0)<_sgLimit}).length};
...
var emails=sgFilterEmails(highCredit.map(function(r){return r.email}).join('\n'));
...
var emails=sgFilterEmails(boughtNoReturn.map(function(c){return c.email}).join('\n'));var _bnCount=sgCountEmails(boughtNoReturn.map(function(c){return c.email}).join('\n'));
if(_bnCount>=1)sg.push({id:'bought-no-return',cat:'retention',priority:'high',title:_bnCount+' buyer'+(_bnCount>1?'s':'')+' absent 30+ days',detail:'Paid customers who have not logged in for over a month.',metrics:[boughtNoReturn.length+' customers'],actions:[{label:'\u2709 Check-in',action:function(){sgSetupEmail({customEmails:emails,brand:'fusion',type:'promotional',from:'tracey@quantumphysician.com',subject:'Your healing content is waiting for you',body:'Hi {{name}},\n\nI noticed you have not visited in a while, and I wanted to **personally reach out**.\n\nYour sessions and courses are still there for you, ready whenever you are. Life gets busy \u2014 I understand. But your healing journey **does not have an expiration date**.\n\nIf you need help accessing your content, just **reply to this email** and I will help.\n\nWith care,\nDr. Tracey Clark'})}}]})}
...
if(topRefs.length>=1){
var emails=sgFilterEmails(topRefs.map(function(r){return r.email}).join('\n'));
