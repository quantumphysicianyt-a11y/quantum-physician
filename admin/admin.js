let currentAdmin=null;const LEGACY_USERNAME='QPadmin',LEGACY_PASSWORD='QPfs#2026';
...
async function doAuth(){var emailEl=document.getElementById('auth-email'),passEl=document.getElementById('auth-pass'),errEl=document.getElementById('auth-error'),statusEl=document.getElementById('auth-status'),btn=document.getElementById('auth-btn');var email=emailEl?emailEl.value.trim():'',pass=passEl?passEl.value:'';if(!email||!pass){errEl.textContent='Enter email and password';errEl.style.display='block';return}errEl.style.display='none';if(email===LEGACY_USERNAME&&pass===LEGACY_PASSWORD){currentAdmin={id:'legacy',email:'admin@qp.local',name:'Admin',role:'super_admin',permissions:{customers:true,email:true,promotions:true,orders:true,community:true,analytics:true,suggestions:true,automation:true,audit:true,system:true,refund:true,delete:true}};sessionStorage.setItem('qp_admin_auth',JSON.stringify(currentAdmin));applyPermissions();document.getElementById('auth-screen').style.display='none';document.getElementById('admin-layout').style.display='block';initAdmin();return}btn.disabled=true;if(statusEl)statusEl.style.display='block';try{var authRes=await sb.auth.signInWithPassword({email:email.toLowerCase(),password:pass});if(authRes.error)throw new Error(authRes.error.message);var admRes=await sbAdmin.from('admin_users').select('*').eq('email',email.toLowerCase()).eq('is_active',true).single();if(admRes.error||!admRes.data)throw new Error('No admin access. Contact the super admin.');await sbAdmin.from('admin_users').update({last_login:new Date().toISOString()}).eq('id',admRes.data.id);currentAdmin={id:admRes.data.id,email:admRes.data.email,name:admRes.data.name,role:admRes.data.role,permissions:{customers:admRes.data.can_customers,email:admRes.data.can_email,promotions:admRes.data.can_promotions,orders:admRes.data.can_orders,community:admRes.data.can_community,analytics:admRes.data.can_analytics,suggestions:admRes.data.can_suggestions,automation:admRes.data.can_automation,audit:admRes.data.can_audit,system:admRes.data.can_system,refund:admRes.data.can_refund,delete:admRes.data.can_delete}};sessionStorage.setItem('qp_admin_auth',JSON.stringify(currentAdmin));applyPermissions();document.getElementById('auth-screen').style.display='none';document.getElementById('admin-layout').style.display='block';initAdmin()}catch(e){errEl.textContent=e.message||'Login failed';errEl.style.display='block'}finally{btn.disabled=false;if(statusEl)statusEl.style.display='none'}}
function doLogout(){sessionStorage.removeItem('qp_admin_auth');currentAdmin=null;location.reload()}
try{var savedAdmin=sessionStorage.getItem('qp_admin_auth');if(savedAdmin&&savedAdmin!=='true'){currentAdmin=JSON.parse(savedAdmin);applyPermissions();document.getElementById('auth-screen').style.display='none';document.getElementById('admin-layout').style.display='block';setTimeout(initAdmin,50)}else if(savedAdmin==='true'){currentAdmin={id:'legacy',email:'admin@qp.local',name:'Admin',role:'super_admin',permissions:{customers:true,email:true,promotions:true,orders:true,community:true,analytics:true,suggestions:true,automation:true,audit:true,system:true,refund:true,delete:true}};applyPermissions();document.getElementById('auth-screen').style.display='none';document.getElementById('admin-layout').style.display='block';setTimeout(initAdmin,50)}}catch(e){sessionStorage.removeItem('qp_admin_auth')}
...
async function logAudit(action,targetEmail,details,metadata){try{var m=metadata||{};var adminName='System';if(currentAdmin){m.admin_email=currentAdmin.email;adminName=currentAdmin.name||currentAdmin.email.split('@')[0]}var res=await sbAdmin.from('admin_audit_log').insert({action:action,target_email:targetEmail?targetEmail.toLowerCase():null,details:details||'',metadata:m,admin_user:adminName});if(res.error)console.error('Audit insert error:',res.error)}catch(e){console.error('Audit log error:',e)}}
...
var sent=0,failed=0;
try{
var campRes=await sbAdmin.from('email_campaigns').insert([{campaign_type:'suggestion',campaign_name:subject,subject:subject,from_email:from,recipient_count:recipients.length,status:'sending'}]).select().single();
...
sent++;
if(campaignId)await sbAdmin.from('email_tracking').insert([{campaign_id:campaignId,recipient_email:r.email,sent_at:new Date().toISOString()}]);
}catch(e){failed++}}
if(campaignId)await sbAdmin.from('email_campaigns').update({status:'sent',sent_count:sent,failed_count:failed}).eq('id',campaignId);
...
var emails=boughtNoReturn.map(function(c){return c.email}).join('\n');
sg.push({id:'bought-no-return',cat:'retention',priority:'high',title:boughtNoReturn.length+' buyer'+(boughtNoReturn.length>1?'s':'')+' absent 30+ days',detail:'Paid customers who have not logged in for over a month.',metrics:[boughtNoReturn.length+' customers'],actions:[{label:'\u2709 Check-in',action:function(){sgSetupEmail({customEmails:emails,brand:'fusion',type:'promotional',from:'tracey@quantumphysician.com',subject:'Your healing content is waiting for you',body:'Hi {{name}},\n\nI noticed you have not visited in a while, and I wanted to **personally reach out**.\n\nYour sessions and courses are still there for you, ready whenever you are. Life gets busy \u2014 I understand. But your healing journey **does not have an expiration date**.\n\nIf you need help accessing your content, just **reply to this email** and I will help.\n\nWith care,\nDr. Tracey Clark'})}}]})}

var optedOut=0;authUsersMap.forEach(function(u){if(u.raw_user_meta_data&&u.raw_user_meta_data.marketing_opt_in===false)optedOut++});
...
function toggleEmailStudentList(filterType){var panel=document.getElementById('email-student-panel');if(panel.style.display!=='none'&&currentEmailStudentFilter===filterType){closeEmailStudentList();return}currentEmailStudentFilter=filterType;var titles={'opted-in':'Opted-In Users','opted-out':'Opted-Out Users','total':'All Users'};document.getElementById('email-student-title').textContent=titles[filterType]||'Users';document.getElementById('email-student-search').value='';panel.style.display='block';var wrap=document.getElementById('email-student-table-wrap');var users=[];authUsersMap.forEach(function(u){var oi=true;if(u.raw_user_meta_data&&u.raw_user_meta_data.marketing_opt_in===false)oi=false;if(filterType==='opted-in'&&!oi)return;if(filterType==='opted-out'&&oi)return;var prof=profilesData.find(function(p){return p.email&&p.email.toLowerCase()===(u.email||'').toLowerCase()});users.push({email:u.email,name:prof?prof.full_name||'':'',optedIn:oi,lastLogin:u.last_sign_in_at})});emailStudentListData=users;renderEmailStudentList(users);panel.scrollIntoView({behavior:'smooth'})}
...
async function updatePromoStatus(promoId,newStatus){var act=newStatus==='inactive'?'deactivate':newStatus==='active'?'reactivate':'archive';if(!await qpConfirm('Confirm','Are you sure you want to '+act+' this promotion?'))return;try{var res=await sbAdmin.from('promotions').update({status:newStatus,updated_at:new Date().toISOString()}).eq('id',promoId);if(res.error)throw new Error(res.error.message);var pr=promotionsData.find(function(p){return p.id===promoId});await logAudit('update_promo',null,(pr?pr.name:'Promo')+' '+act+'d',{promo_id:promoId,new_status:newStatus});await loadAllData();loadPromotionsPage();showToast('Promotion '+act+'d','success')}catch(e){showToast('Error: '+e.message,'error')}}
async function deletePromotion(promoId,name){if(!await qpConfirm('Delete Promotion','Permanently delete "'+name+'"?',{danger:true,okText:'Delete'}))return;try{var pr=promotionsData.find(function(p){return p.id===promoId});await sbAdmin.from('promotions').delete().eq('id',promoId);await logAudit('delete_promo',null,'Deleted: '+name,{promo_id:promoId,coupon_id:pr?pr.coupon_id:''});await loadAllData();loadPromotionsPage();showToast('Promotion deleted','success')}catch(e){showToast('Error: '+e.message,'error')}}
/* ===== ORDERS (SESSION 5) ===== */
...
var card2Cta=card2Inner.indexOf('Share')>=0||card2Inner.indexOf('referral')>=0||card2Inner.indexOf('Referral')>=0?'GO TO REFERRAL HUB':(card2Inner.indexOf('code')>=0?'CLAIM YOUR DISCOUNT':'EXPLORE SESSIONS');
var card2Url=card2Cta==='GO TO REFERRAL HUB'?siteUrl+'/referral-hub.html':(discountConfig&&discountConfig.couponId?siteUrl+'/?coupon='+discountConfig.couponId:siteUrl);
discountCardHtml+='<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin:20px 0 5px;"><tr><td style="background:linear-gradient(135deg,#1a0533,#0f0f23);border:2px solid #8338ec;border-radius:16px;padding:25px 20px;text-align:center;"><p style="margin:0;font-size:15px;color:#fff;line-height:1.6;">'+card2Inner+'</p>'+qr2Html+'<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:18px auto 0;"><tr><td style="background:linear-gradient(45deg,#8338ec,#00f5ff);border-radius:50px;"><a href="'+wrapLink(card2Url)+'" target="_blank" style="display:inline-block;padding:14px 40px;color:#fff;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:2px;text-transform:uppercase;">'+card2Cta+'</a></td></tr></table><p style="margin:10px 0 0;font-size:11px;color:#666;">Applied automatically at checkout</p></td></tr></table>'
}
...
if(!ok) return;
try{var r=await sbAdmin.from('scheduled_emails').update({scheduled_for:new Date().toISOString()}).eq('id',id);
...
if(!ok) return;
try{var r=await sbAdmin.from('scheduled_emails').update({status:'cancelled'}).eq('id',id);
if(r.error) throw r.error;
...
if(!ok) return;
try{var r=await sbAdmin.from('scheduled_emails').update({status:'pending',scheduled_for:new Date().toISOString(),sent_count:0,failed_count:0,updated_at:new Date().toISOString()}).eq('id',id);
if(r.error) throw r.error;
...
if(!ok) return;
try{var r=await sbAdmin.from('scheduled_emails').update({status:'pending',scheduled_for:new Date().toISOString(),sent_count:0,failed_count:0,sent_at:null,updated_at:new Date().toISOString()}).eq('id',id);
if(r.error) throw r.error;
...
try{var r=await sbAdmin.from('scheduled_emails').update({status:'pending',scheduled_for:parsed.toISOString(),updated_at:new Date().toISOString()}).eq('id',id);
if(r.error) throw r.error;
...
try{var r=await sbAdmin.from('scheduled_emails').update({subject:subject,body:body,audience:audience,scheduled_for:new Date(datetime).toISOString(),updated_at:new Date().toISOString()}).eq('id',id);
if(r.error) throw r.error;
...
try{var savedAdmin2=sessionStorage.getItem('qp_admin_auth');if(savedAdmin2){if(savedAdmin2!=='true')currentAdmin=JSON.parse(savedAdmin2);else currentAdmin={id:'legacy',email:'admin@qp.local',name:'Admin',role:'super_admin',permissions:{customers:true,email:true,promotions:true,orders:true,community:true,analytics:true,suggestions:true,automation:true,audit:true,system:true,refund:true,delete:true}};applyPermissions();document.getElementById('auth-screen').style.display='none';document.getElementById('admin-layout').style.display='block';initAdmin()}}catch(e){sessionStorage.removeItem('qp_admin_auth')}
...
try{
var res=await sbAdmin.from('promotions').update(updates).eq('id',promoId);
...
try{
var res=await sbAdmin.from('scheduled_emails').insert({
...
try{
var res=await sbAdmin.from('session_schedule').update(updates).eq('id',sessionId);
if(res.error)throw new Error(res.error.message);
...
{name:'Bold CTA',icon:'zap',desc:'Strong call to action',body:"**Ready to Take the Next Step?**\n\nYour transformation is waiting. Whether you're continuing your journey or just beginning, now is the perfect time."},
...
var iconSvg=auditSvg(card.icon,'var(--teal)');
