...
async function webhookRecovery(){var modal=document.createElement('div');modal.id='webhook-recovery-modal';modal.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.8);z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px';modal.onclick=function(e){if(e.target===modal)modal.remove()};var box=document.createElement('div');box.style.cssText='background:var(--navy-deep);border:1px solid rgba(91,168,178,.25);border-radius:16px;padding:28px;width:95%;max-width:900px;max-height:90vh;overflow-y:auto;position:relative';box.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><h2 style="margin:0;color:var(--teal);font-family:Playfair Display,serif">Webhook Recovery Tool</h2><button onclick="this.closest(\'#webhook-recovery-modal\').remove()" style="background:rgba(91,168,178,.1);border:1px solid rgba(91,168,178,.25);color:var(--teal);font-size:18px;cursor:pointer;width:34px;height:34px;border-radius:50;display:flex;align-items:center;justify-content:center;padding:0">\u00d7</button></div><p style="color:var(--text-dim);font-size:13px;margin-bottom:16px">Paste Stripe customer emails (one per line) with optional amount. Format: <code style="color:var(--teal)">email|amount|product</code> or just <code style="color:var(--teal)">email</code></p><textarea id="wr-input" class="input" style="width:100%;height:150px;font-family:monospace;font-size:12px" placeholder="sandrawenrich1@gmail.com|500|bundle-all\nsullivan12@gmail.com|50|session-1"></textarea><div style="display:flex;gap:8px;margin-top:12px"><button class="btn" style="background:var(--teal);color:#fff" onclick="wrAnalyze()">Analyze</button><button class="btn" style="background:rgba(91,168,178,.1);color:var(--teal);border:1px solid rgba(91,168,178,.25)" onclick="wrLoadStripeData()">Load Known Missing</button></div><div id="wr-results" style="margin-top:20px"></div>';modal.appendChild(box);document.body.appendChild(modal)}

function wrLoadStripeData(){var data='sandrawenrich1@gmail.com|500|bundle-all\nmariejowett@btinternet.com|500|bundle-all\njscottacctsvs@gmail.com|500|bundle-all\nknovak.can@gmail.com|500|bundle-all\nhighlander4tunes@yahoo.ca|500|bundle-all\nrdwells34@hotmail.com|500|bundle-all\ncharla.lower@gmail.com|500|bundle-all\nsookhan.lee@gmail.com|500|bundle-all\njeannemaken@gmail.com|500|bundle-all\nquantumwhisperer@icloud.com|500|bundle-all\njokearney@hotmail.com|500|bundle-all\ngayle622@montana.com|500|bundle-all\ncmpare@hotmail.ca|500|bundle-all\nd_pszeniczny@hotmail.com|500|bundle-all\ntriplejequine@outlook.com|500|bundle-all\nbttammy5@gmail.com|500|bundle-all\nsullivan12@gmail.com|50|session-1\nbrucekruger@sasktel.net|50|session-1\nhaya.ht.farah@gmail.com|50|session-1\nmichellewinandy@gmail.com|50|session-1\nteresa.ohlson@yahoo.com|50|session-1\nheartbeatofmotherearth1@hotmail.com|50|session-1\necfpryor@gmail.com|50|session-1\nchristel.erni@mac.com|50|session-1\nksp587p50@gmail.com|50|session-1\ncherylcoetzee241@gmail.com|50|session-1\ntrishamcphillips08@hotmail.co.uk|45|session-1\nnfhughes@yahoo.co.uk|45|session-1\ngnusllc@gmail.com|50|session-1\nhwhitelaw4@gmail.com|50|session-1\nlivsimonsen@outlook.dk|50|session-1\nmdlpayrollacc@gmail.com|50|session-1\nkat@katsaunders.co.uk|45|session-1\nlaurika.bodytalk@gmail.com|50|session-1';document.getElementById('wr-input').value=data;showToast('Loaded '+data.split('\n').length+' customers from Stripe data','info')}

async function wrAnalyze(){var input=document.getElementById('wr-input').value.trim();if(!input){showToast('Paste email data first','error');return}var lines=input.split('\n').filter(function(l){return l.trim()});var results=[];for(var i=0;i<lines.length;i++){var parts=lines[i].trim().split('|');var email=parts[0].trim().toLowerCase();var amount=parts[1]?parseFloat(parts[1]):0;var product=parts[2]?parts[2].trim():(amount>=500?'bundle-all':'session-1');var cust=allCustomers.find(function(c){return c.email.toLowerCase()===email});var hasPurchase=purchasesData.some(function(p){return p.email&&p.email.toLowerCase()===email});var hasAuth=authUsersMap.has(email);var hasProfile=profilesData.some(function(p){return p.email&&p.email.toLowerCase()===email});results.push({email:email,amount:amount,product:product,hasCustomer:!!cust,hasPurchase:hasPurchase,hasAuth:hasAuth,hasProfile:hasProfile,needsFix:!hasPurchase})}var missing=results.filter(function(r){return r.needsFix});var ok=results.filter(function(r){return!r.needsFix});var html='<div style="margin-bottom:16px"><span style="color:var(--success);font-weight:600">'+ok.length+' OK</span> &nbsp; <span style="color:var(--danger);font-weight:600">'+missing.length+' Missing Purchases</span></div>';if(missing.length){html+='<div style="margin-bottom:12px"><label style="display:flex;align-items:center;gap:8px;color:var(--text-dim);font-size:13px;cursor:pointer"><input type="checkbox" id="wr-send-email" checked> Send purchase confirmation email</label></div>';html+='<table class="tbl"><thead><tr><th><input type="checkbox" id="wr-select-all" checked onchange="wrToggleAll(this.checked)"></th><th>Email</th><th>Amount</th><th>Product</th><th>Auth</th><th>Profile</th><th>Status</th></tr></thead><tbody>';missing.forEach(function(r,idx){html+='<tr><td><input type="checkbox" class="wr-check" data-idx="'+idx+'" checked></td><td style="font-size:12px">'+r.email+'</td><td>$'+r.amount+'</td><td style="font-size:11px">'+productName(r.product)+'</td><td>'+(r.hasAuth?'\u2705':'\u274c')+'</td><td>'+(r.hasProfile?'\u2705':'\u274c')+'</td><td><span class="badge" style="background:rgba(255,107,107,.15);color:var(--danger)">No Purchase Record</span></td></tr>'});html+='</tbody></table>';html+='<div style="display:flex;gap:8px;margin-top:16px"><button class="btn btn-success" onclick="wrGrantAll()">Grant Access to Selected ('+missing.length+')</button></div>'}if(ok.length){html+='<details style="margin-top:16px"><summary style="color:var(--text-dim);cursor:pointer;font-size:13px">'+ok.length+' customers already have purchase records</summary><div style="margin-top:8px;max-height:200px;overflow-y:auto">';ok.forEach(function(r){html+='<div style="padding:4px 0;font-size:12px;color:var(--text-dim);border-bottom:1px solid rgba(255,255,255,.04)">\u2705 '+r.email+'</div>'});html+='</div></details>'}document.getElementById('wr-results').innerHTML=html;window._wrMissing=missing}

function wrToggleAll(checked){document.querySelectorAll('.wr-check').forEach(function(cb){cb.checked=checked})}

async function wrGrantAll(){var missing=window._wrMissing;if(!missing)return;var selected=[];document.querySelectorAll('.wr-check:checked').forEach(function(cb){selected.push(missing[parseInt(cb.dataset.idx)])});if(!selected.length){showToast('No customers selected','error');return}var sendEmail=document.getElementById('wr-send-email').checked;if(!await qpConfirm('Webhook Recovery','Grant access to '+selected.length+' customer(s)?'+(sendEmail?' A purchase confirmation email will be sent to each.':''),{okText:'Grant All'}))return;var log=document.getElementById('wr-results');var statusDiv=document.createElement('div');statusDiv.style.cssText='margin-top:16px;padding:16px;background:rgba(0,0,0,.2);border-radius:8px;max-height:300px;overflow-y:auto';log.appendChild(statusDiv);var success=0,failed=0;for(var i=0;i<selected.length;i++){var r=selected[i];statusDiv.innerHTML+='<div style="padding:3px 0;font-size:12px;color:var(--text-dim)">\u23f3 Processing '+r.email+'...</div>';statusDiv.scrollTop=statusDiv.scrollHeight;try{var pid=r.product;if(pid==='bundle-all'){var bundleProducts=['bundle-all','session-1','session-2','session-3','session-4','session-5','session-6','session-7','session-8','session-9','session-10','session-11','session-12'];for(var j=0;j<1;j++){await sb.from('purchases').upsert({email:r.email,product_id:pid,stripe_event_id:'webhook-recovery-'+Date.now()+'-'+i,amount_paid:r.amount,purchased_at:new Date().toISOString()},{onConflict:'stripe_event_id'})}}else{await sb.from('purchases').upsert({email:r.email,product_id:pid,stripe_event_id:'webhook-recovery-'+Date.now()+'-'+i,amount_paid:r.amount,purchased_at:new Date().toISOString()},{onConflict:'stripe_event_id'})}await logAudit('grant_access',r.email,'Webhook recovery: granted '+productName(pid)+' ($'+r.amount+')',{product_id:pid,recovery:true});if(sendEmail){var name=r.email.split('@')[0].replace(/[._]/g,' ').replace(/\b\w/g,function(c){return c.toUpperCase()});var subject='Your Quantum Physician Purchase Confirmation';var body='Hi '+name+',\n\nThank you for your purchase of '+productName(r.product)+'. We apologize for the delay in getting your confirmation \u2014 we experienced a brief technical issue that has now been resolved.\n\nYour access is now fully active. Here\u2019s what you purchased:\n\n\u2022 '+productName(r.product)+'\n\u2022 Amount: $'+r.amount+'\n\nYou can access your sessions and content by logging into your dashboard.\n---\nIf you don\u2019t yet have an account, visit fusionsessions.com and create one using this same email address ('+r.email+'). Your purchased content will be waiting for you.\n---\nQuestions? Simply reply to this email \u2014 we\u2019re here to help.\n\nWith gratitude,\nDr. Tracey Clark';var richHtml=buildRichEmail(body,null,'https://fusionsessions.com',null);try{await fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain'},body:JSON.stringify({to:r.email,subject:subject,body:richHtml,isHtml:true,from:'Quantum Physician <traceyclarkcbi@gmail.com>'})});statusDiv.innerHTML+='<div style="padding:3px 0;font-size:12px;color:var(--success)">\u2709 Email sent to '+r.email+'</div>'}catch(emailErr){statusDiv.innerHTML+='<div style="padding:3px 0;font-size:12px;color:var(--warning)">\u26a0 Access granted but email failed for '+r.email+'</div>'}}statusDiv.innerHTML+='<div style="padding:3px 0;font-size:12px;color:var(--success)">\u2705 '+r.email+' \u2014 access granted'+(r.product==='bundle-all'?' (Full Bundle)':'')+' $'+r.amount+'</div>';success++}catch(e){statusDiv.innerHTML+='<div style="padding:3px 0;font-size:12px;color:var(--danger)">\u274c '+r.email+' \u2014 '+e.message+'</div>';failed++}statusDiv.scrollTop=statusDiv.scrollHeight}statusDiv.innerHTML+='<div style="padding:8px 0;margin-top:8px;border-top:1px solid rgba(255,255,255,.1);font-size:14px;font-weight:600;color:var(--teal)">\u2705 Done: '+success+' granted, '+failed+' failed</div>';await loadAllData();showToast('Recovery complete: '+success+' granted','success')}
