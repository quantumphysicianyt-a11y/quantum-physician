...
/* Auto-refresh: update stored token whenever Supabase refreshes the session */
...
var _sessionRestorePromise=(async function(){try{
  ...
  else if(tok){var s=await sb.auth.getSession();if(s.data&&s.data.session&&s.data.session.refresh_token)sessionStorage.setItem('qp_admin_refresh',s.data.session.refresh_token)}
...
setInterval(async function(){
  try{
    var ref=sessionStorage.getItem('qp_admin_refresh');
    if(ref){
      var r=await sb.auth.refreshSession({refresh_token:ref});
      if(r.data&&r.data.session){
        sessionStorage.setItem('qp_admin_token',r.data.session.access_token);
        if(r.data.session.refresh_token)sessionStorage.setItem('qp_admin_refresh',r.data.session.refresh_token);
      }
    }else{
...
function go(page,btn){
  _reCleanupPickers();_ecEditor=null;if(_reInstances["ec"])delete _reInstances["ec"];currentPage=page;sessionStorage.setItem('qp_admin_page',page);document.querySelectorAll('.sb-link').forEach(l=>l.classList.remove('active'));if(btn)btn.classList.add('active');else{document.querySelectorAll('.sb-link').forEach(function(l){if(l.getAttribute('onclick')&&l.getAttribute('onclick').indexOf("'"+page+"'")!==-1)l.classList.add('active')});}document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));var el=document.getElementById('page-'+page);if(el)el.classList.add('active');document.getElementById('topbar-title').textContent=TITLES[page]||page;loadPageData(page);document.getElementById('sidebar').classList.remove('open');document.getElementById('sb-overlay').classList.remove('open')}
...
async function loginAsUser(email,dest){var destName=dest==='academy'?'Academy':'Fusion Sessions';if(!await qpConfirm('Login As User','Generate a magic login link for '+email+' on '+destName+'?',{okText:'Generate Link'}))return;try{var redirectUrl=dest==='academy'?'https://academy.quantumphysician.com':'https://fusionsessions.com';var fetchRes=await authAdminAPI('generate_link',{type:'magiclink',email:email,options:{redirect_to:redirectUrl}});var res={ok:true};var data=fetchRes.data;var link=data.properties&&data.properties.action_link||data.action_link;await logAudit('login_as',email,'Login As on '+destName,{destination:dest});if(link){window.open(link,'_blank')}else{showToast('Could not generate link','error')}}catch(e){showToast('Error: '+e.message,'error')}}
...
/* --- AUDIT LOG PAGE --- */
var auditPage=1,auditPS=25,auditSortCol='created_at',auditSortDir='desc';
...
html+='<button class="btn btn-sm" style="background:var(--success-soft);color:var(--success)" onclick="withSpinner(this,function(){return sendNowSched(\''+email.id+'\')})">Send Now</button>';
html+='<button class="btn btn-sm" style="background:var(--danger-soft);color:var(--danger)" onclick="withSpinner(this,function(){return cancelSchedEmail(\''+email.id+'\')})">Cancel</button>';
}
if(email.status==='failed') html+='<button class="btn btn-sm" style="background:var(--warning-soft);color:var(--warning)" onclick="withSpinner(this,function(){return retrySchedEmail(\''+email.id+'\')})">Retry</button>';
if(email.status==='sent') html+='<button class="btn btn-sm" style="background:var(--warning-soft);color:var(--warning)" onclick="withSpinner(this,function(){return resendSchedEmail(\''+email.id+'\')})">Resend</button>';
if(email.status==='cancelled') html+='<button class="btn btn-sm" style="background:var(--success-soft);color:var(--success)" onclick="withSpinner(this,function(){return reactivateSchedEmail(\''+email.id+'\')})">Reactivate</button>';
html+='</div></div></div>'
});
...
function sortAuditLog(col){if(auditSortCol===col){auditSortDir=auditSortDir==='asc'?'desc':'asc';}else{auditSortCol=col;auditSortDir=col==='created_at'?'desc':'asc';}auditPage=1;loadAuditLog();}
...
+'<div style="display:flex;gap:10px;justify-content:flex-end"><button class="btn btn-ghost" onclick="document.getElementById(\'edit-promo-overlay\').remove()">Cancel</button><button class="btn btn-primary" onclick="withSpinner(this,function(){return saveEditPromotion(\''+promoId+'\')})">Save Changes</button></div>';
...
+'<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px"><button class="btn btn-ghost" onclick="document.getElementById(\'create-sched-overlay\').remove()">Cancel</button><button class="btn btn-primary" onclick="withSpinner(this,function(){return saveScheduledEmail()})">Schedule Email</button></div>';
...
+'<div style="display:flex;gap:10px;justify-content:flex-end"><button class="btn btn-ghost" onclick="document.getElementById(\'edit-sess-overlay\').remove()">Cancel</button><button class="btn btn-primary" onclick="withSpinner(this,function(){return saveSessionSchedule(\''+sessionId+'\')})">Save</button></div>';
...
/* ---------- CTA Button Insert ---------- */
var EC_CTA_BUTTONS={
  watch:{label:'Watch Your Sessions',url:'https://fusionsessions.com/dashboard.html'},
  dashboard:{label:'Go to Dashboard',url:'https://fusionsessions.com/dashboard.html'},
  referral:{label:'Visit Referral Hub',url:'https://fusionsessions.com/referral-hub.html'},
  ...
  if(!sessTypesData.length){
    ...
    sessTypesData.forEach(function(t){
      ...
      html+='<div style="display:flex;justify-content:space-between;align-items:center;padding:14px;border:1px solid var(--border);border-radius:var(--radius-sm);margin-bottom:8px;background:rgba(0,0,0,.1)">'
        ...
        +'<button class="btn btn-ghost btn-sm" style="color:var(--danger)" onclick="withSpinner(this,function(){return deleteSessionType(\''+t.id+'\')})">Delete</button>'
        +'</div></div>';
    });
    el.innerHTML=html;
  }
...
function showAddSessionTypeModal(existingId){
  ...
  box.innerHTML='<div style="font-weight:600;font-size:16px;margin-bottom:16px;color:var(--teal)">'+(existing?'Edit':'Add')+' Session Type</div>'
    ...
    +'<button class="btn btn-primary btn-sm" onclick="withSpinner(this,function(){return saveSessionType('+(existing?"'"+existing.id+"'":'null')+')})">Save</button></div>';
  ov.appendChild(box);document.body.appendChild(ov);
}
...
var CONFIRM_WINDOW_MS=7*24*60*60*1000; // 7 days production
  ...
  if(status==='client_confirmation'&&cycle.client_confirmation_sent_at){
    ...
    if(remaining>0){
      ...
      return '<span id="sess-confirm-countdown" style="display:inline-flex;align-items:center;gap:8px;margin-left:8px">'
        ...
        +'<button class="btn btn-ghost btn-sm sess-advance-glow" onclick="withSpinner(this,function(){return advanceCycleStatus(\''+id+'\',\''+status+'\')})">'+label+'</button>'
        +'</span>';
    }
  }
  ...
  if(status==='waitlist_open'&&cycle.waitlist_expires_at){
    ...
    if(wlRemaining>0){
      ...
      return '<span style="display:inline-flex;align-items:center;gap:8px;margin-left:8px">'
        ...
        +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return advanceCycleStatus(\''+id+'\',\''+status+'\')})">'+label+' (skip wait)</button>'
        +'</span>';
    }
  }
...
function renderCyclesList(){
  ...
  c.innerHTML=sessCyclesData.map(function(cy){
    ...
    return'<div onclick="selectCycle(\''+cy.id+'\')" style="padding:14px 16px;border:2px solid '+(isSelected?borderColors[cy.status]:'var(--border)')+';border-radius:var(--radius-sm);margin-bottom:8px;background:'+(isSelected?'rgba(91,168,178,.06)':'rgba(0,0,0,.1)')+';cursor:pointer;transition:all .15s">'
      ...
      +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return advanceCycleStatus(\''+cy.id+'\',\''+cy.status+'\')})">Advance →</button>':'')
      +'<button class="btn btn-danger btn-sm" onclick="withSpinner(this,function(){return deleteCycle(\''+cy.id+'\',\''+esc(cy.name.replace(/'/g,"\\'"))+'\')})">Delete</button>'
      +'</div></div></div>';
  }).join('');
}
...
function renderClientRoster(){
  ...
  var sortVal=(document.getElementById('sess-client-sort')||{}).value||'priority';
  var clients=sessClientsData.slice();
  ...
  clients.sort(function(a,b){
    switch(sortVal){
      ...
      default:return(a.priority||0)-(b.priority||0);
    }
  });
...
async function editClient(id){
  ...
  box.innerHTML='<div style="font-weight:600;margin-bottom:14px">Edit Client: '+esc(cl.email)+'</div>'
    ...
    +'<div style="display:flex;gap:8px;justify-content:flex-end"><button class="btn btn-ghost btn-sm" onclick="document.getElementById(\'sess-edit-client-modal\').remove()">Cancel</button><button class="btn btn-primary btn-sm" onclick="withSpinner(this,function(){return saveEditClient(\''+id+'\')})">Save</button></div>';
  ov.appendChild(box);document.body.appendChild(ov);
}
...
function renderPublicWaitlist(){
  ...
  configEl.innerHTML='<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">'
    ...
    +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return togglePublicBooking(\'open\')})">Open</button>'
    +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return togglePublicBooking(\'coming_soon\')})">Coming Soon</button>'
    +'<button class="btn btn-danger btn-sm" onclick="withSpinner(this,function(){return togglePublicBooking(\'closed\')})">Close</button>'
    +'</div>';
...
async function offerSlot(waitlistId,email){
  ...
  box.innerHTML='<div style="font-weight:600;font-size:16px;margin-bottom:4px;color:var(--teal)">'+(isRegular?'Confirm Slot for ':'Offer Slot to ')+esc(wl?wl.name:email)+'</div>'
    ...
    +'<button class="btn btn-primary btn-sm" onclick="withSpinner(this,function(){return sendOfferSlot(\''+waitlistId+'\',\''+esc(email)+'\')})">Send Offer</button></div>';
...
async function crmOpenClient(email){
  ...
  try {
    ...
    var sessClient = sessClientsData.find(function(c){ return c.email && c.email.toLowerCase() === emailLower; });
    var name = (profile && profile.full_name) || clientInfo.name || (sessClient && sessClient.name) || emailLower.split('@')[0];
...
function renderCrmSessions(){
  ...
  // Wire up edit buttons via delegation (avoids escaping issues in onclick)
  el.querySelectorAll('.note-edit-btn').forEach(function(btn){
    btn.onclick = function(){
      var nid = btn.getAttribute('data-nid');
      var note = crmNotes.find(function(n){ return n.id === nid; });
      if(note) crmEditNote(nid, note.content, note.body_regions || [], !!note.visible_to_patient);
    };
  });
}
...
async function crmToggleNoteVisibility(noteId, visible){
  try{ await proxyFrom('session_notes').update({ visible_to_patient: visible }).eq('id', noteId); showToast(visible ? 'Note shared with patient' : 'Note set to private', 'success'); if(crmCurrentClient) await crmOpenClient(crmCurrentClient); await refreshBookingsView(); } catch(e){ showToast('Error updating note: '+e.message, 'error'); }
}
...
async function crmEditNote(noteId, currentContent, currentRegions, currentVisible){
  return new Promise(function(resolve){
    ...
    o.className = 'modal-overlay';
    var html = '<div class="modal-box" style="max-width:620px;max-height:90vh;overflow-y:auto">';
    html += '<div class="modal-title">Edit Session Note</div>';
    html += '<div style="margin-bottom:14px"><label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px">Session Notes</label>';
    html += '<textarea id="edit-note-text" class="input" style="width:100%;min-height:100px;resize:vertical;font-size:13px">' + esc(currentContent) + '</textarea></div>';
    ...
    BODY_REGION_OPTIONS.forEach(function(section){
      ...
      html += '<div style="display:flex;flex-wrap:wrap;gap:5px">';
      section.regions.forEach(function(r){
        var sel = currentRegions && currentRegions.indexOf(r.id) > -1 ? ' selected' : '';
        html += '<button type="button" class="region-chip' + sel + '" data-rid="' + r.id + '" onclick="this.classList.toggle(\'selected\')" style="padding:4px 10px;font-size:11px;border-radius:20px;border:1px solid var(--border);background:transparent;color:var(--text-muted);cursor:pointer;transition:all .15s">' + esc(r.label) + '</button>';
      });
      html += '</div></div>';
    });
    html += '</div>';
    ...
    html += '<div style="margin-bottom:16px;padding:10px 12px;background:rgba(0,0,0,.15);border-radius:8px;display:flex;align-items:center;gap:12px">';
    html += '<div style="flex:1"><div style="font-size:13px;font-weight:600;color:var(--text)">Share with Patient</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px">Patient will see this note on their progress page</div></div>';
    html += '<button type="button" id="edit-note-visible-btn" class="' + (currentVisible ? 'on' : '') + '" onclick="this.classList.toggle(\'on\');this.textContent=this.classList.contains(\'on\')?\'YES\':\'NO\'" style="min-width:52px;padding:6px 12px;border-radius:6px;border:1px solid var(--border);background:transparent;color:var(--text-muted);font-size:12px;font-weight:700;cursor:pointer;transition:all .15s">' + (currentVisible ? 'YES' : 'NO') + '</button>';
    html += '</div>';

    html += '<div class="modal-actions"><button class="btn btn-ghost btn-sm" id="edit-cancel">Cancel</button><button class="btn btn-primary btn-sm" id="edit-save">Save Changes</button></div>';
    html += '</div>';
    o.innerHTML = html;

    var style = document.createElement('style');
    style.textContent = '.region-chip.selected{background:var(--teal)!important;color:#fff!important;border-color:var(--teal)!important;font-weight:600} #edit-note-visible-btn.on{background:var(--teal)!important;color:#fff!important;border-color:var(--teal)!important}';
    o.querySelector('.modal-box').prepend(style);
    document.body.appendChild(o);
    document.getElementById('edit-note-text').focus();

    document.getElementById('edit-save').onclick = async function(){
      var content = document.getElementById('edit-note-text').value.trim();
      if(!content){ showToast('Please enter a note', 'warning'); return; }
      var chips = o.querySelectorAll('.region-chip.selected');
      var regions = [];
      chips.forEach(function(c){ regions.push(c.getAttribute('data-rid')); });
      var visible = document.getElementById('edit-note-visible-btn').classList.contains('on');
      o.remove();
      try{
    ...
    document.getElementById('edit-cancel').onclick = function(){ o.remove(); resolve(); };
    o.onclick = function(e){ if(e.target===o){ o.remove(); resolve(); }};
  });
}
...
async function crmDeleteNote(noteId){
  ...
  try{ await proxyFrom('session_notes').delete().eq('id', noteId); showToast('Note deleted', 'success'); if(crmCurrentClient) await crmOpenClient(crmCurrentClient); await refreshBookingsView(); } catch(e){ showToast('Error: '+e.message, 'error'); }
}
  ...
  if(clientInvoices.length){
    ...
    sorted.forEach(function(inv){
      ...
      html += '<tr>'
        ...
        +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return resendInvoice(\''+inv.id+'\')" style="font-size:11px})">📧</button>'
        +'</div></td></tr>';
    });
...
function renderSessionReminders(){
  ...
  if(upcoming.length){
    ...
    upcoming.forEach(function(b){
      ...
      if(needsReminder){
        html += '<button class="btn btn-primary btn-sm" onclick="withSpinner(this,function(){return sendSessionReminder(\'day-before\',\''+b.id+'\',\''+esc(b.email)+'\')})">Send Reminder</button>';
      }
  ...
  if(completed.length){
    ...
    completed.forEach(function(b){
      ...
      html += '<div style="display:flex;gap:4px">';
      html += '<button class="btn btn-success btn-sm" onclick="withSpinner(this,function(){return sendSessionReminder(\'follow-up\',\''+b.id+'\',\''+esc(b.email)+'\')})">Send Follow-Up</button>';
  ...
  c.innerHTML='<table class="tbl"><thead><tr>'
    ...
    +page.map(function(inv){
      ...
      return '<tr>'
        ...
        +'<button class="btn btn-ghost btn-sm" onclick="withSpinner(this,function(){return resendInvoice(\''+inv.id+'\')" style="font-size:11px})">📧 Email</button>'
        +'</div></td></tr>';
    }).join('')+'</tbody></table>';
...
async function createInvoiceModal(bookingId){
  ...
  box.innerHTML='<div style="font-weight:600;font-size:16px;margin-bottom:16px;font-family:Playfair Display,serif">Create Invoice</div>'
    ...
    +'<button class="btn btn-primary" onclick="withSpinner(this,function(){return saveInvoice(null, \''+((booking&&booking.id)||'')+'\')})">Save Draft</button>'
    +'<button class="btn btn-success" onclick="withSpinner(this,function(){return saveAndSendInvoice(null, \''+((booking&&booking.id)||'')+'\')})">Generate PDF & Send</button>'
    +'</div>';
  ov.appendChild(box);document.body.appendChild(ov);
}
...
async function editInvoiceModal(invId){
  ...
  box.innerHTML='<div style="font-weight:600;font-size:16px;margin-bottom:4px;font-family:Playfair Display,serif">Edit Invoice</div>'
    ...
    +'<div style="display:flex;gap:8px;justify-content:flex-end">'
    +'<button class="btn btn-ghost" onclick="document.getElementById(\'inv-modal\').remove()">Cancel</button>'
    +'<button class="btn btn-primary" onclick="withSpinner(this,function(){return saveInvoice(\''+inv.id+'\',\''+(inv.booking_id||'')+'\')})">Save</button>'
    +'<button class="btn btn-success" onclick="withSpinner(this,function(){return saveAndSendInvoice(\''+inv.id+'\',\''+(inv.booking_id||'')+'\')})">Save & Send</button>'
    +'</div>';
  ov.appendChild(box);document.body.appendChild(ov);
}
