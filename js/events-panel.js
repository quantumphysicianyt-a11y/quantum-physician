/* =============================================
   QUANTUM ACADEMY — EVENTS PANEL v4
   ============================================= */
(function(){
  ...
  function detectCategory(title, desc){
    ...
    if(t.match(/fundamental/)) return {label:'Fundamentals', color:'#34d399', bg:'rgba(52,211,153,.12)'};
  ...
  css.textContent = [
    /* Overlay */
    '.ep-overlay{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9998;opacity:0;transition:opacity .3s;pointer-events:none;backdrop-filter:blur(2px)}',
    ...
    /* Full Calendar button - prominent */
    ...
    /* ===== FULL CALENDAR MODAL ===== */
    '.fc-modal{position:fixed;inset:0;z-index:10000;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.65);backdrop-filter:blur(4px)}',
    '.fc-modal.open{display:flex}',
    '.fc-box{background:linear-gradient(170deg,#0d2740 0%,#081d30 40%,#071825 100%);border:1px solid rgba(91,168,178,.2);border-radius:16px;width:940px;max-width:94vw;max-height:90vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 80px rgba(0,0,0,.6),0 0 40px rgba(91,168,178,.05),inset 0 1px 0 rgba(255,255,255,.03)}',
    /* Header with branding */
    '.fc-top{display:flex;align-items:center;justify-content:space-between;padding:18px 24px;border-bottom:1px solid rgba(91,168,178,.12);background:linear-gradient(135deg,rgba(91,168,178,.06),rgba(74,207,217,.02),transparent)}',
    '.fc-brand{display:flex;align-items:center;gap:14px}',
    '.fc-brand img{height:28px;width:auto;opacity:.7}',
    '.fc-brand h2{font-family:"Playfair Display",serif;font-size:22px;color:rgba(255,255,255,.9)}',
    '.fc-brand .fc-sub{font-size:10px;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.1em;margin-top:1px}',
    '.fc-nav{display:flex;align-items:center;gap:8px}',
    '.fc-nbtn{width:32px;height:32px;border-radius:8px;background:rgba(255,255,255,.03);border:1px solid rgba(91,168,178,.15);color:rgba(255,255,255,.5);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s}',
    '.fc-nbtn:hover{border-color:rgba(91,168,178,.4);color:#5ba8b2;background:rgba(91,168,178,.06)}',
    '.fc-nbtn svg{width:14px;height:14px;stroke:currentColor}',
    '.fc-today{padding:6px 14px;background:linear-gradient(135deg,rgba(91,168,178,.12),rgba(74,207,217,.08));border:1px solid rgba(91,168,178,.3);color:#4acfd9;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;transition:all .15s}',
    '.fc-today:hover{background:linear-gradient(135deg,rgba(91,168,178,.2),rgba(74,207,217,.12));box-shadow:0 2px 8px rgba(74,207,217,.15)}',
    /* Grid */
    '.fc-grid{padding:16px 24px;overflow-y:auto;flex:1}',
    '.fc-dow{display:grid;grid-template-columns:repeat(7,1fr);text-align:center;font-size:10px;font-weight:700;color:rgba(91,168,178,.5);text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px}',
    '.fc-days{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}',
    '.fc-day{min-height:85px;padding:8px;border-radius:8px;border:1px solid transparent;transition:all .2s;cursor:default;background:rgba(255,255,255,.01)}',
    '.fc-day:hover{border-color:rgba(91,168,178,.12);background:rgba(91,168,178,.02)}',
    '.fc-day .fc-num{font-size:13px;color:rgba(255,255,255,.25);margin-bottom:4px;font-weight:500}',
    '.fc-day.cur .fc-num{color:rgba(255,255,255,.7)}',
    '.fc-day.today-cell{background:rgba(91,168,178,.06);border-color:rgba(91,168,178,.2);box-shadow:inset 0 0 20px rgba(91,168,178,.04)}',
    '.fc-day.today-cell .fc-num{color:#4acfd9;font-weight:700;font-size:14px}',
    '.fc-day.has-ev{cursor:pointer}',
    '.fc-day.has-ev:hover{border-color:rgba(91,168,178,.25);background:rgba(91,168,178,.04);transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.15)}',
    '.fc-ev{font-size:10px;padding:3px 6px;border-radius:4px;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer;font-weight:600;transition:opacity .15s}',
    '.fc-ev:hover{opacity:.8}',
    '.fc-day-more{font-size:9px;color:rgba(91,168,178,.4);margin-top:2px;font-weight:600}',
    /* Day detail panel */
    '.fc-detail{padding:16px 24px;border-top:1px solid rgba(91,168,178,.12);max-height:280px;overflow-y:auto;background:linear-gradient(180deg,rgba(91,168,178,.03),transparent)}',
    '.fc-detail h3{font-family:"Playfair Display",serif;font-size:17px;margin-bottom:12px;color:rgba(255,255,255,.88)}',
    /* Legend */
    '.fc-legend{display:flex;gap:12px;padding:10px 24px;border-top:1px solid rgba(91,168,178,.08);flex-wrap:wrap}',
    '.fc-leg{display:flex;align-items:center;gap:4px;font-size:9px;color:rgba(255,255,255,.35);font-weight:600;text-transform:uppercase;letter-spacing:.04em}',
    '.fc-leg-dot{width:8px;height:8px;border-radius:2px}',
    /* Light theme */
    '[data-theme="light"] .ep-panel{background:linear-gradient(180deg,#f7f5f0,#f0ede6);box-shadow:-8px 0 30px rgba(0,0,0,.1)}',
    '[data-theme="light"] .ep-head{background:linear-gradient(135deg,rgba(173,155,132,.05),transparent)}',
    '[data-theme="light"] .ep-head h2,[data-theme="light"] .ep-title{color:#1a1a2e}',
    '[data-theme="light"] .ep-card{background:linear-gradient(135deg,#fff,#faf9f7);border-color:rgba(173,155,132,.15)}',
    '[data-theme="light"] .ep-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.06)}',
    '[data-theme="light"] .ep-time,[data-theme="light"] .ep-desc,[data-theme="light"] .ep-hint{color:rgba(26,26,46,.5)}',
    '[data-theme="light"] .ep-detail,[data-theme="light"] .ep-month-label{border-color:rgba(173,155,132,.15)}',
    '[data-theme="light"] .fc-box{background:linear-gradient(170deg,#f7f5f0,#f0ede6);border-color:rgba(173,155,132,.2);box-shadow:0 24px 80px rgba(0,0,0,.15)}',
    '[data-theme="light"] .fc-top{border-color:rgba(173,155,132,.15);background:linear-gradient(135deg,rgba(173,155,132,.05),transparent)}',
    '[data-theme="light"] .fc-top h2,[data-theme="light"] .fc-detail h3{color:#1a1a2e}',
    '[data-theme="light"] .fc-day{background:rgba(0,0,0,.01)}',
    '[data-theme="light"] .fc-day:hover{background:rgba(173,155,132,.04)}',
    '[data-theme="light"] .fc-day .fc-num{color:rgba(26,26,46,.25)}',
    '[data-theme="light"] .fc-day.cur .fc-num{color:#1a1a2e}',
    '[data-theme="light"] .fc-day.today-cell{background:rgba(91,168,178,.05)}',
  ...
  panel.innerHTML = '<div class="ep-head"><h2>Upcoming Events</h2><div class="ep-head-btns"><button class="ep-fullcal-btn" onclick="openFullCalendar()" title="Full Calendar">'+icons.cal+' Full Calendar</button><button class="ep-hbtn" onclick="closeEventsPanel()">\u2715</button></div></div><div class="ep-body" id="epBody"><div class="ep-spin"></div></div>';
  document.body.appendChild(panel);

  var fcModal = document.createElement('div');
  fcModal.className = 'fc-modal';
  fcModal.innerHTML = '<div class="fc-box">'
    +'<div class="fc-top"><div class="fc-brand"><img src="/assets/images/QuantumPhysAcad_STACKED-RGB.png" alt="" onerror="this.style.display=\'none\'"><div><h2 id="fcTitle"></h2><div class="fc-sub">Quantum Academy Schedule</div></div></div>'
    +'<div class="fc-nav"><button class="fc-today" onclick="fcGoToday()">Today</button><button class="fc-nbtn" onclick="fcNav(-1)">'+icons.left+'</button><button class="fc-nbtn" onclick="fcNav(1)">'+icons.right+'</button><button class="fc-nbtn" onclick="closeFullCalendar()" style="margin-left:8px">\u2715</button></div></div>'
  ...
  function loadPanel(){
    var now=new Date();
    fetchEvents(new Date(now.getFullYear(),now.getMonth(),now.getDate()-7),new Date(now.getFullYear(),now.getMonth()+3,0),function(events){
      allEvents=events;renderPanel(events);
    });
  }
  ...
  function renderPanel(events){
    ...
    if(recent.length){
      html+='<div class="ep-month-label" style="color:rgba(255,255,255,.35)">Recent</div>';
  ...
  function renderCard(ev, isPast){
    ...
    h+='</div></div>';
    h+='<div class="ep-hint">tap for details</div></div>';
    return h;
  }
  ...
  function renderFullCal(){
    ...
    for(var d=1;d<=daysInMonth;d++){
      ...
      dayEv.slice(0,3).forEach(function(ev){
        var cat=detectCategory(ev.title,ev.description);
        catsSeen[cat.label]={color:cat.color,bg:cat.bg};
        html+='<div class="fc-ev" style="background:'+cat.bg+';color:'+cat.color+';border-left:2px solid '+cat.color+'">'+ev.title+'</div>';
      });
      if(dayEv.length>3)html+='<div class="fc-day-more">+'+(dayEv.length-3)+' more</div>';
      html+='</div>';
    }
    var total=startDay+daysInMonth;var rem=total%7===0?0:7-total%7;
    ...
    // Legend
    var leg='';
    Object.keys(catsSeen).forEach(function(k){
      leg+='<div class="fc-leg"><div class="fc-leg-dot" style="background:'+catsSeen[k].color+'"></div>'+k+'</div>';
    });
